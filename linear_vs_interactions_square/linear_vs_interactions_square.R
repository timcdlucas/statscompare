...
covs_clean2 <- model.matrix( ~-1 + .^2, data = covs_clean)
...
plotCV(m2, alpha = 0.1) + 
  xlim(0, 0.5) +
  ggtitle(paste0('Interaction and linear terms. MAE = ', err2)) + 
  xlab('Predicted value') + 
  ylab('Observed value')
ggsave('interaction_terms.png', height = 4, width = 4)
